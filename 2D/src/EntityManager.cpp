...
void EntityManager::removeEntity(const std::string& tag, const std::shared_ptr<Entity>& entity)
{
	auto& entities = mEntitiesMap.at(tag);
	try{
	for(auto e: entities)
	{
		if (e == entity)
	...
	{
		std::cout << "No entities with tag " << tag << std::endl;
	}
}
